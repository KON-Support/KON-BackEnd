...
INSERT INTO tb_chamados (DSTITULO, DSDESCRICAO, STATUS, SOLICITANTE, RESPONSAVEL, ANEXO, CATEGORIA, PLANO, SLA, DTCRIACAO, DTFECHAMENTO, DTVENCIMENTO, FLSLAVIOLADO)
SELECT 'Erro ao acessar o sistema', 'Usuario nao consegue logar no sistema', 'ABERTO', 1, 3, NULL, 1, 1, 1, NOW(), NULL, NOW() + INTERVAL '48 HOURS', FALSE
WHERE NOT EXISTS (SELECT 1 FROM tb_chamados WHERE DSTITULO = 'Erro ao acessar o sistema' AND SOLICITANTE = 1);

INSERT INTO tb_chamados (DSTITULO, DSDESCRICAO, STATUS, SOLICITANTE, RESPONSAVEL, ANEXO, CATEGORIA, PLANO, SLA, DTCRIACAO, DTFECHAMENTO, DTVENCIMENTO, FLSLAVIOLADO)
SELECT 'Solicitacao de novo acesso', 'Criar usuario para novo colaborador', 'ABERTO', 1, 3, NULL, 2, 1, 2, NOW(), NULL, NOW() + INTERVAL '24 HOURS', FALSE
